...
window.addEventListener('load', () => {
    ...
    // Mark document elements with desktop mode class
    if (window.isDesktopMode) {
        document.documentElement.classList.add('is-desktop');
        document.body.classList.add('is-desktop');
        
        // Prevent touch controls from being added
        // Override any functions that might add touch controls
        window.forceMobileControls = false;
        
        // Override appendChild to prevent adding touch controls
        const originalAppendChild = Element.prototype.appendChild;
        Element.prototype.appendChild = function(child) {
            // If we're in desktop mode and this looks like a control element, don't append it
            if (window.isDesktopMode && child) {
                // Check if it's an image that contains control assets
                if (child.tagName === 'IMG' && child.src && 
                    (child.src.includes('arrow.png') || child.src.includes('shoot.png'))) {
                    console.log('Blocked touch control from being added:', child);
                    return child; // Return but don't actually append
                }
                
                // Check for control IDs
                if (child.id && 
                    (child.id.includes('btn') || child.id.includes('control') || 
                     child.id === 'mobile-controls-overlay' || child.id === 'ultra-stable-controls')) {
                    console.log('Blocked touch control container from being added:', child.id);
                    return child; // Return but don't actually append
                }
            }
            
            // Otherwise, proceed with normal appendChild
    ...
    function addStandaloneControls() {
        ...
        // Check global desktop mode flag first
        if (window.isDesktopMode === true) {
            console.log('Desktop mode detected via global flag - skipping standalone controls');
            
            // Ensure controls are cleaned up
    ...
    // Force iframe mode detection and mark document body
    document.addEventListener('DOMContentLoaded', function() {
      ...
      if (isIframe) {
        document.body.classList.add('iframe-mode');
        console.log('Iframe mode detected - activating stable controls');
      }
      
      // Forcibly add the class after a short delay regardless of detection
      setTimeout(function() {
        document.body.classList.add('iframe-mode');
        
        // Force fixed positions for buttons
        const buttons = ['left-btn', 'right-btn', 'shoot-btn'];
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      setTimeout(function() {
        ...
        if (window.gameInstance) {
          // Override the addOnScreenControls method to use our stable controls
...
iframeStyle.textContent = `
    ...
    /* IMPORTANT: Always hide controls in desktop mode */
    html.is-desktop #left-btn, 
    html.is-desktop #right-btn, 
    html.is-desktop #shoot-btn,
    html.is-desktop #mobile-controls-overlay, 
    html.is-desktop #ultra-stable-controls,
    html.is-desktop img[src*="left_arrow.png"], 
    html.is-desktop img[src*="right_arrow.png"], 
    html.is-desktop img[src*="shoot.png"],
    html.is-desktop .mobile-control, 
    html.is-desktop .touch-button, 
    html.is-desktop .control-button {
...
// Mark the HTML element with desktop class if needed
if (window.isDesktopMode === true) {
    document.documentElement.classList.add('is-desktop');
}
...
function createStableControlOverlay() {
  // Check for desktop mode multiple ways to ensure touch controls don't appear
  const isDesktopByClass = document.documentElement.classList.contains('is-desktop');
  const isDesktopByWindow = window.isDesktopMode === true;
  const isDesktopByForceMobile = window.forceMobileControls === false;
  
  // Check all desktop indicators - if ANY indicate desktop, don't create controls
  if (isDesktopByClass || isDesktopByWindow || isDesktopByForceMobile) {
    console.log("Desktop mode detected - completely skipping stable controls");
    
    // Remove any existing controls using a thorough approach
    function removeAllControls() {
      // Remove by ID - standard controls
      ['left-btn', 'right-btn', 'shoot-btn', 'ultra-stable-controls', 
      'mobile-controls-overlay', 'mobile-controls', 'iframe-controls-display'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.remove();
      });
      
      // Remove by image source - control assets
      document.querySelectorAll('img[src*="left_arrow.png"], img[src*="right_arrow.png"], img[src*="shoot.png"]').forEach(el => el.remove());
      
      // Remove all fixed position elements at bottom of screen (likely controls)
      document.querySelectorAll('div[style*="fixed"][style*="bottom"], img[style*="fixed"][style*="bottom"]').forEach(el => el.remove());
      
      // Remove by contains selector for any remaining controls
      document.querySelectorAll('[id*="btn"], [id*="control"], [class*="control"], [class*="mobile"]').forEach(el => el.remove());
    }
    
    // Run cleanup immediately
    removeAllControls();
    
    // Run cleanup again after a short delay to catch anything that appears dynamically
    setTimeout(removeAllControls, 100);
    setTimeout(removeAllControls, 500);
    setTimeout(removeAllControls, 1000);
    
    return null; // Exit without creating controls
  }
  
  // Proceed to mobile controls creation logic for mobile devices only
  // ... existing code ...
}
