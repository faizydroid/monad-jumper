...
if (typeof window !== 'undefined') {
  window.__GLOBAL_TX_SYSTEM = {
    ...
    canSendTransaction: function(key) {
      console.log(`🔒 TX CHECK: ${key} - ActiveTxs: ${this.activeTransactions.size}, History: ${this.txHistory.size}`);
    ...
    failTransaction: function(key, error) {
      ...
      this.activeTransactions.delete(key);
      this.pendingLock = false;
      console.log(`❌ TX FAILED: ${key} - ${error}`);
    },
  ...
  // Log function to track all transaction activity
...
function GameComponent({ hasMintedNft, isNftLoading, onOpenMintModal, onGameOver }) {
  ...
  // Add this near the top global variables to track all game sessions
  if (typeof window !== 'undefined') {
    // Add specific session tracking to completely prevent duplicate transactions
    window.__PROCESSED_GAME_SESSIONS = new Set();
    window.__PENDING_TRANSACTIONS = new Map();
    
    // Create a global single-transaction enforcer
    window.__enforceOnlyOneTransaction = function(gameId, callback) {
      // If this game session has already been processed, block all transactions
      if (window.__PROCESSED_GAME_SESSIONS.has(gameId)) {
        console.log(`🛑 BLOCKED TRANSACTION: Game session ${gameId} already processed`);
        return false;
      }
      
      // Record this game session as processed immediately
      window.__PROCESSED_GAME_SESSIONS.add(gameId);
      console.log(`✅ REGISTERING GAME SESSION: ${gameId} as processed`);
      
      // Track pending transaction
      window.__PENDING_TRANSACTIONS.set(gameId, {
        status: 'processing',
        startTime: Date.now()
      });
      
      // Execute the callback with session tracking
      try {
        console.log(`🔒 EXECUTING EXCLUSIVE TRANSACTION for game session ${gameId}`);
        return callback();
      } catch (error) {
        console.error(`❌ ERROR during exclusive transaction for ${gameId}:`, error);
        return false;
      }
    };
  }
  
  // Centralized function for recording jumps with duplicate prevention
  const recordPlayerJumps = useCallback(async (jumpCount, jumpGameId, source = "unknown") => {
    console.log(`⛔ BLOCKED: recordPlayerJumps called from ${source} - using central handler instead`);
    return false;
  }, []);
  ...
  // Completely disable any other game over handlers
  const handleGameOver = useCallback(async (score) => {
    console.log("⛔ BLOCKED: handleGameOver called - using central handler instead");
    return false;
  }, []);
  ...
  // Completely disable the recordPlayerJumps function to prevent it from being used
  const recordPlayerJumps = useCallback(async (jumpCount, jumpGameId, source = "unknown") => {
    console.log(`⛔ BLOCKED: recordPlayerJumps called from ${source} - using central handler instead`);
    return false;
  }, []);

  // Replace the handleGameMessages function with this completely rewritten version
  const handleGameMessages = useCallback(async (event) => {
    // Skip if not from our iframe
    if (!iframeRef.current || event.source !== iframeRef.current.contentWindow) return;
    
    // CRITICAL: Create one central handler for all transaction-related events
    const gameData = event.data;
    
    // Ignore anything that's not game over or bundle jumps
    if (!gameData || typeof gameData !== 'object') return;
    if (gameData.type !== 'GAME_OVER' && 
        gameData.type !== 'gameOver' && 
        gameData.type !== 'BUNDLE_JUMPS') return;
    
    console.log(`📣 TRANSACTION EVENT RECEIVED: ${gameData.type}`);
    
    // Normalize game session ID from any source
    const gameSessionId = 
      gameData.gameId || 
      gameData.data?.gameId || 
      gameData.data?.sessionId || 
      window.__currentGameSessionId ||
      gameId;
    
    // Normalize score and jumps from any source
    const finalScore = 
      gameData.score || 
      gameData.data?.finalScore || 
      gameData.data?.score || 
      0;
    
    const jumpCount = 
      gameData.jumps || 
      gameData.data?.jumpCount || 
      gameData.data?.jumps ||
      window.__jumpCount || 
      0;
    
    console.log(`🎮 Game data: session=${gameSessionId}, score=${finalScore}, jumps=${jumpCount}`);
    
    // CRITICAL: Only allow one transaction per game session using our enforcer
    window.__enforceOnlyOneTransaction(gameSessionId, async () => {
      // Skip if wallet not connected
      if (!address || !walletClient || !publicClient) {
        console.log("⛔ Cannot send transaction - wallet not connected");
        return false;
      }
      
      // Skip if no jumps
      if (jumpCount <= 0) {
        console.log("⚠️ No jumps to record, skipping transaction");
        setShowPlayAgain(true);
        return false;
      }
      
      try {
        console.log(`🚀 SENDING SINGLE TRANSACTION for session ${gameSessionId}`);
        setTransactionPending(true);
        
        // Use the combined function that handles both jumps and score in ONE transaction
        ...
        // Add a delay to ensure UI updates first
        await new Promise(resolve => setTimeout(resolve, 800));
        
        // Use explicit gas parameters to avoid estimation delays
        const hash = await walletClient.writeContract({
          address: contractAddress,
          abi: gameAbi,
          functionName: 'recordJumpsAndScore',
          args: [BigInt(jumpCount), BigInt(finalScore)],
          account: address,
          gas: BigInt(500000),
        ...
        console.log(`📤 SENT COMBINED TRANSACTION: ${hash} for session ${gameSessionId}`);
        ...
        // Record success
        window.__PENDING_TRANSACTIONS.set(gameSessionId, {
          status: 'completed',
        ...
        console.error(`❌ TRANSACTION ERROR for session ${gameSessionId}:`, error);
  ...
  }, [address, walletClient, publicClient, gameId, setTransactionPending, setShowPlayAgain]);
  ...
  const handleMessageFromGame = useCallback((event) => {
    ...
    if (event.data && typeof event.data === 'object') {
      if (event.data.type === 'gameOver') {
        ...
        // DON'T call handleGameOver here - let the main handler deal with it
    ...
    const handleIframeMessage = async (event) => {
      ...
      if (data && typeof data === 'object') {
        ...
        if (data.type === 'REVIVE_CANCELLED') {
          ...
          // CRITICAL: Don't handle transactions here, let the central handler do it
          console.log('⏩ Forwarding to central transaction handler');
          
          // Transform this into a GAME_OVER event for the central handler
        ...
        // All other event handling can remain unchanged
        // ...
      }
    };
  ...
  useEffect(() => {
    ...
    const handleGameMessage = async (event) => {
      ...
      // Check if it's a reload_clicked message - keep this part unchanged
